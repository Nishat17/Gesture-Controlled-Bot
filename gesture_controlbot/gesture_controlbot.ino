...
void loop()
{
  ...
  //z = analogRead(zPin); //Reads Z coordinates (Not Used)
    if(x<377&&y<340)      // Change the value for adjusting sensitivity  
      forward();
    else if(x>277&&y<378) // Change the value for adjusting sensitivity
      backward();
    else if(x>331&&y>266) // Change the value for adjusting sensitivity
      right();
    else if(y<394) // Change the value for adjusting sensitivity
      left();
    else
      stop_();
}
